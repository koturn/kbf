...
private:
  ...
  {
    // skip header
    skip(sizeof(Elf64_Ehdr) + sizeof(Elf64_Phdr) * kNProgramHeaders);
  ...
  emitFooterImpl() CODE_GENERATOR_NOEXCEPT
  {
    ...
    std::size_t codeSize = static_cast<std::size_t>(oStream.tellp()) - kHeaderSize;
