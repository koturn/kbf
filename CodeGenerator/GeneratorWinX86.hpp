...
protected:
  ...
  emitFooterImpl() CODE_GENERATOR_NOEXCEPT
  {
    ...
    write(static_cast<u32>(ioh.ImageBase + iids[0].FirstThunk));
    oStreamPtr->seekp(ishText.PointerToRawData + 0x08, std::ios_base::beg);
    write(static_cast<u32>(ioh.ImageBase + iids[0].FirstThunk + sizeof(DWORD)));
    oStreamPtr->seekp(exitAddrPos, std::ios_base::beg);
    write(static_cast<u32>(ioh.ImageBase + iids[0].FirstThunk + sizeof(DWORD) * 2));
    oStreamPtr->seekp(ishText.PointerToRawData + 0x0d, std::ios_base::beg);
    write(static_cast<u32>(ioh.ImageBase + ishBss.VirtualAddress));
